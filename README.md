...
## 📌 LiveData와 Coroutine의 역할 구분 (추가 학습)

처음엔 Room DB를 사용할 때 모든 작업을 반드시 Coroutine으로 처리해야 한다고 생각 했었지만, 조회 방식에 따라 처리 방식이 달라진다는 점을 학습했습니다.

✔️ 1. LiveData를 반환하는 조회
```kotlin
@Query("SELECT * FROM TodoData")
fun getAllData(): LiveData<List<TodoEntity>>
```
- Room이 내부적으로 백그라운드 스레드에서 데이터를 조회
- 데이터 변경 시 자동으로 UI에 반영 (Observer 패턴)
- 별도의 Coroutine 없이 observe()만으로 처리 가능
...
➡️ LiveData 기반 조회는 Coroutine 없이도 비동기적으로 동작

### ✔️ 2. 데이터 변경 (Insert / Delete / Update)
```kotlin
@Insert
...
```
### ✔️ 3. suspend 함수를 사용하는 조회

```kotlin
@Query("SELECT * FROM TodoData")
suspend fun getAllData(): List<TodoEntity>
```

- LiveData가 아닌 일반 List를 반환하는 경우
- Coroutine을 사용하여 비동기 처리 필요
...
처음에는 모든 DB 작업을 Coroutine으로 처리해야 한다고 생각했지만,
LiveData를 활용하면 조회 작업은 Room이 내부적으로 비동기 처리 및 데이터 관찰을 수행한다는 점을 이해하게 되었습니다.
